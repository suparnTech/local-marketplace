// app/shop-owner/(tabs)/_layout.tsx - Kinetic Navigation Portal (Shop Owner)
...
const getIconName = (routeName: string, isFocused: boolean): any => {
    const iconMap: Record<string, { focused: string; unfocused: string }> = {
        index: { focused: 'stats-chart', unfocused: 'stats-chart-outline' },
        products: { focused: 'cube', unfocused: 'cube-outline' },
        orders: { focused: 'receipt', unfocused: 'receipt-outline' },
        profile: { focused: 'person', unfocused: 'person-outline' }, // Future proofing
    };
...
function KineticTabBar({ state, descriptors, navigation }: any) {
    ...
    const VISIBLE_TAB_NAMES = ['index', 'orders', 'products', 'profile'];
        ...
        <View style={styles.tabBarWrapper}>
            <BlurView intensity={80} tint="dark" style={styles.tabBarContainer}>
                ...
                <Animated.View style={[styles.indicator, indicatorStyle]}>
                    <LinearGradient
                        colors={[colors.primary, colors.primaryDark]} // Use primary color for Shop Owner
...
export default function ShopOwnerTabsLayout() {
    ...
    // Activate real-time notifications for the shop owner
    // It will auto-fetch shopId since role is STORE_OWNER
    useNotifications(user?.id, undefined, user?.role);
